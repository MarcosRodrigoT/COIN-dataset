...
for youtube_id in data:
    ...
    url = info["video_url"]
    trans_loc = output_path + "/" + str(type)
    if not os.path.exists(trans_loc):
        os.mkdir(trans_loc)

    cmd = f"yt-dlp --write-auto-sub --skip-download --sub-format 'srt' --sub-lang 'en' -o '{trans_loc}/{youtube_id}' {url}"
    os.system(cmd)
